        ...
        <q-form @submit="save()">
          <q-card-section>
            ...
            <q-select
              ...
              :options="estadoList"
              :rules="[value => value || 'Favor selecionar um estado']"
            />
          </q-card-section>
          <q-separator/>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        { name: "estado_id", label: "UF", align: "left", field: row => row.estado.uf, sortable: true },
  ...
  methods: {
    request() {
      ...
      this.$store.dispatch('cidade/list')
        .then(() => {
          this.rows = this.$store.getters['cidade/list'];
        })
        .catch(() => {
          console.log('erro');
        })
    ...
    edit(id) {
      ...
      this.$store.dispatch('cidade/show', id).then(() => {
        this.formData = Object.assign({}, this.$store.getters['cidade/byId']);
        this.showFormModal = true;
      }).catch(() => {
        this.$q.notify({
          color  : 'negative',
    ...
    save() {
      ...
      this.$store.commit('cidade/setData', { data: this.formData });
      this.$store.dispatch(`cidade/${methodAction}`).then(() => {
        this.$q.notify({
          color  : 'positive',
    ...
    remove() {
      ...
      this.$store.dispatch(`cidade/${methodAction}`, this.id).then(() => {
        this.$q.notify({
          color  : 'positive',
    ...
    async loadEstados() {
      ...
      try {
        await this.$store.dispatch('estado/listOptions');
        this.estadoList = this.$store.getters['estado/options'];
      } catch (err) {
        this.$q.notify({
          color  : 'negative',
